...
class Suppliers:
    def __init__(self, initial_suppliers=None):
        if initial_suppliers is None:
            self.suppliers = []
        else:
            self.suppliers = initial_suppliers

    def __str__(self):
        supplier_regions = ""
        for supplier in self.suppliers:
            supplier_regions += f"{supplier}\n"
        return supplier_regions

    def print_supplier_regions(self):
        for supplier in self.suppliers:
            print(supplier.get_region())

    def print_supplier_regions_with_colon_numbers(self):
        for i, supplier in enumerate(self.suppliers, start=1):
            print(f"{i}: {supplier.get_region()}")

    def print_supplier_regions_with_dot_numbers(self):
        for i, supplier in enumerate(self.suppliers, start=1):
            print(f"{i}. {supplier.get_region()}")

    def get_supplier_from_index(self, index):
        try:
            return self.suppliers[index - 1]
        except IndexError:
            raise IndexError("No supplier found at that index!")

    def get_by_region(self, region):
        for supplier in self.suppliers:
            if supplier.get_region() == region:
                return supplier
        return None
